...
if(point.code!==selectedGbPoint.code){pinFocus(null);setNeedleReport(null);}setLocatorActive(current=>current&&hasLocatorItems(point.code));setPointQuery(point.code);setState(s=>({...s,selected:[],isolate:false,rotate:false,needle:{enabled:true,depthRatio:0,revision:(s.needle?.revision??0)+1},acupuncture:{visible:true,selectedCode:point.code,showAll:false,showLines:false},regionFocus:zoom?{center:point.seed,radiusMm:focusRadiusMm(point,zoom),viewHint:point.projection==='dorsal-foot'?'dorsal-foot':undefined,revision:(s.regionFocus?.revision??0)+1}:s.regionFocus}));setDetails(false);if(zoom)revealScene();};
 ...
 return <main className={`studio ${needleEyeOpen?'needle-eye-analysis':''} ${needleEyeWorkspace?'needle-eye-workspace':''} ${compact?'is-compact':''}`} style={{'--inset-top':`${viewInsets.top}px`,'--inset-right':`${viewInsets.right}px`,'--inset-bottom':`${viewInsets.bottom}px`,'--inset-left':`${viewInsets.left}px`} as React.CSSProperties}>
  ...
  <nav className="top-actions" aria-label="Explorer panels"><Button variant="ghost" className={panel==='search'?'active':''} onClick={()=>openPanel('search')} aria-label="Search anatomy"><Search size={18}/><span>Find a structure</span><kbd>/</kbd></Button><Button variant="ghost" className="icon-button" data-tutorial="help" aria-label="탐색 튜토리얼 다시 보기" title="튜토리얼 다시 보기" onClick={replayTutorial}><HelpCircle size={18}/></Button><Button variant="ghost" className="icon-button" aria-label="About this atlas" onClick={()=>{setDetails(false);setPanel(null);setAbout(true);}}><Info size={18}/></Button></nav>
  ...
  <nav className="view-controls glass" aria-label="Camera controls">
   {(['three-quarter','front','side','back'] as View[]).map((v,i)=><Button variant="ghost" key={v} className={state.view===v?'active':''} aria-pressed={state.view===v} disabled={state.explode>.8&&v!=='front'} onClick={()=>{setState(s=>({...s,view:v,reset:s.reset+1,rotate:false}));tutorial.advance('rotate','zoom');}} title={`${v} view`} aria-label={`${v} view`}><span>{['¾','F','S','B'][i]}</span></Button>)}
   <i/>
   <Button variant="ghost" aria-label="확대" title="확대" onClick={()=>zoomScene(.78)}><ZoomIn size={18}/></Button>
   <Button variant="ghost" aria-label="축소" title="축소" onClick={()=>zoomScene(1.28)}><ZoomOut size={18}/></Button>
   <i/>
   <Button variant="ghost" disabled={state.explode>=.4} aria-label={state.rotate?'Pause rotation':'Rotate body'} title="Auto rotate" className={state.rotate?'active':''} onClick={()=>{setState(s=>({...s,rotate:!s.rotate}));tutorial.advance('rotate','zoom');}}>{state.rotate?<Pause size={17}/>:<RotateCw size={18}/>}</Button>
  ...
  <nav className="mobile-camera-controls glass" aria-label="모바일 카메라 조작">
   <Button variant="ghost" aria-label="몸 돌리기" title="몸 돌리기" onClick={()=>{setState(s=>({...s,view:s.view==='front'?'three-quarter':'front',reset:s.reset+1,rotate:false}));tutorial.advance('rotate','zoom');}}><RotateCw size={18}/></Button>
   <Button variant="ghost" aria-label="확대" title="확대" onClick={()=>zoomScene(.78)}><ZoomIn size={18}/></Button>
   <Button variant="ghost" aria-label="축소" title="축소" onClick={()=>zoomScene(1.28)}><ZoomOut size={18}/></Button>
  </nav>
