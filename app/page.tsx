...
import {Activity,ArrowLeft,ArrowRight,ArrowUpRight,ChevronDown,ChevronRight,ChevronUp,Focus,HelpCircle,Info,Layers3,Move,RotateCcw,RotateCw,Search,Sparkles,Undo2,X,ZoomIn,ZoomOut} from 'lucide-react';
...
import {DEFAULT_VISIBLE,SYSTEMS,EXPLANATIONS,explanation,bilingualPartName,withSupplement,type Atlas,type Concept,type NeedleReport,type SceneState,type Supplement,type SystemId} from './anatomy';
...
import {ButtonTips} from './button-tips';
...
import {Tutorial,FeatureHint,MiniTour,MINI_TOURS,type MiniTourId} from './tutorial';
import {useTutorial} from './use-tutorial';
import './needle-eye.css';
import './tutorial.css';
const initial:SceneState={explode:0,visible:DEFAULT_VISIBLE,selected:[],isolate:false,view:'three-quarter',rotate:false,reset:0,needle:{enabled:true,depthRatio:0,revision:0},acupuncture:{visible:true,selectedCode:'GB34',showAll:true}};
...
// Hangul syllables split into jamo, so a syllable still being composed matches the name it is on its way to.
const CHO='ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎ',JUNG='ㅏㅐㅑㅒㅓㅔㅕㅖㅗㅘㅙㅚㅛㅜㅝㅞㅟㅠㅡㅢㅣ',JONG=['','ㄱ','ㄲ','ㄱㅅ','ㄴ','ㄴㅈ','ㄴㅎ','ㄷ','ㄹ','ㄹㄱ','ㄹㅁ','ㄹㅂ','ㄹㅅ','ㄹㅌ','ㄹㅍ','ㄹㅎ','ㅁ','ㅂ','ㅂㅅ','ㅅ','ㅆ','ㅇ','ㅈ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ'];
const syllable=(char:string)=>{const code=char.charCodeAt(0)-0xac00;return code>=0&&code<11172?code:-1;};
const jamo=(text:string)=>[...text.replace(/\s+/g,'')].map(char=>{const code=syllable(char);return code<0?char:CHO[Math.floor(code/588)]+JUNG[Math.floor(code%588/28)]+JONG[code%28];}).join('');
const initials=(text:string)=>[...text].map(char=>{const code=syllable(char);return code<0?char:CHO[Math.floor(code/588)];}).join('');
...
export default function Home(){
 const detailTitle=useRef<HTMLHeadingElement>(null);
  ...
  const tutorial=useTutorial(progress===100&&!!atlas,compact);
  const zoomRevision=useRef(0);
  ...
  const toggleNeedleEye=()=>{if(needleEyeOpen){closeNeedleEye();return;}setLocatorActive(false);setPanel(p=>p==='layers'?null:p);setNeedleEyeOpen(true);tutorial.openMiniTourOnce('needle-eye');};
  ...
  const adjustNeedleFromCompass=(direction:-1|1)=>{tutorial.advance('needle-eye','finish');if(direction>0&&needle.depthRatio>=100)setStrataRefusal(value=>value+1);updateNeedle(Math.max(0,Math.min(100,needle.depthRatio+direction*5)));const panelElement=acupuncturePanel.current,simulation=needlePanel.current;if(!panelElement||!simulation)return;const offset=simulation.getBoundingClientRect().top-panelElement.getBoundingClientRect().top;panelElement.scrollTo({top:Math.max(0,panelElement.scrollTop+offset-12),behavior:'smooth'});};
...
if(point.code!==selectedGbPoint.code){pinFocus(null);setNeedleReport(null);}setLocatorActive(current=>current&&hasLocatorItems(point.code));setPointQuery(point.code);setState(s=>({...s,selected:[],isolate:false,rotate:false,needle:{enabled:true,depthRatio:0,revision:(s.needle?.revision??0)+1},acupuncture:{visible:true,selectedCode:point.code,showAll:false},regionFocus:zoom?{center:point.seed,radiusMm:focusRadiusMm(point,zoom),viewHint:point.projection==='dorsal-foot'?'dorsal-foot':undefined,revision:(s.regionFocus?.revision??0)+1}:s.regionFocus}));setDetails(false);if(zoom)revealScene();};
  const toggleLocator=()=>{if(locatorActive){setLocatorActive(false);return;}tutorial.advance('locator','play');tutorial.openMiniTourOnce('locator');closeNeedleEye();selectAcupoint(hasLocatorItems(selectedGbPoint.code)?selectedGbPoint.code:'KI4');if(compact)setSheetDetent('peek');else if(window.innerWidth<900)setPanelCollapsed(true);setLocatorRevision(value=>value+1);setLocatorActive(true);};
  // "전체 경혈 보기": keep the selection, show the whole meridian and return the camera to the overview.
  const showWholeMeridian=()=>{setState(s=>({...s,acupuncture:{visible:true,selectedCode:selectedGbPoint.code,showAll:true},regionFocus:undefined,reset:s.reset+1}));revealScene();};
 ...
 useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.key==='/'&&!(e.target instanceof HTMLInputElement)&&!(e.target instanceof HTMLTextAreaElement)){e.preventDefault();setPanel('search');setDetails(false);tutorial.openMiniTourOnce('search');}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);},[tutorial.openMiniTourOnce]);
 useEffect(()=>{const key=(e:KeyboardEvent)=>{const target=e.target;if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement||target instanceof HTMLSelectElement||(target instanceof HTMLElement&&target.isContentEditable))return;if(e.key==='Escape'&&!e.defaultPrevented&&!panel&&!details&&!about&&!document.querySelector('[role=dialog]')){e.preventDefault();showWholeMeridian();}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);},[selectedGbPoint.code,panel,details,about]);
  ...
  const compassFocus={pinned:pinnedFocus,preview:previewFocus,pin:(focus:StructureFocus|null)=>{pinFocus(focus);if(focus?.pinned&&focus.source==='text'&&focus.label.includes('비골'))tutorial.advance('landmark','locator');},partIdsFor};
 ...
 // Acupoints lead the search: by WHO code (GB34, gb-34, GB034), Korean name (양릉천) or hanja (陽陵泉).
 const pointResults=useMemo(()=>{
  const raw=query.trim();if(!raw)return [] as AcupointDefinition[];
  const compact=raw.toUpperCase().replace(/[\s-]+/g,''),code=/^([A-Z]+)0*(\d*)$/.exec(compact),wanted=code?`${code[1]}${code[2]}`:null;
  // Korean is compared letter by letter (jamo): while the IME is still composing, "삼음" arrives as "삼으",
  // which is not a substring of "삼음교" but is a jamo prefix of it. Initials alone (ㅅㅇㄱ) also match.
  const typed=jamo(raw),initialsOnly=/^[ㄱ-ㅎ]+$/.test(raw);
  const rank=(point:AcupointDefinition)=>{
   if(wanted&&point.code===wanted)return 0;
   if(point.korean===raw||point.hanja===raw)return 0;
   if(wanted&&point.code.startsWith(wanted))return 1;
   const name=jamo(point.korean);
   // Whole syllables first: "삼" should list 삼음교 before 사만, which only matches letter by letter.
   if(point.korean.startsWith(raw)||point.hanja.startsWith(raw))return 1.5;
   if(name.startsWith(typed))return 2;
   if(name.includes(typed)||point.hanja.includes(raw))return 3;
   if(initialsOnly&&initials(point.korean).startsWith(raw))return 4;
   if(/^[a-z\s-]{3,}$/i.test(raw)&&point.english.toLowerCase().includes(raw.toLowerCase()))return 5;
   return -1;
  };
  return allPoints.map(point=>({point,score:rank(point)})).filter(item=>item.score>=0).sort((a,b)=>a.score-b.score||a.point.code.localeCompare(b.point.code,undefined,{numeric:true})).slice(0,12).map(item=>item.point);
 },[allPoints,query]);
 type SearchItem={kind:'point';id:string;point:AcupointDefinition}|{kind:'concept';id:string;concept:Concept};
 const searchItems=useMemo<SearchItem[]>(()=>[...pointResults.map(point=>({kind:'point' as const,id:`point:${point.code}`,point})),...results.map(concept=>({kind:'concept' as const,id:concept.id,concept}))],[pointResults,results]);
 const chooseSearchItem=(item:SearchItem)=>{if(item.kind==='concept'){choose(item.concept);return;}setPanel(null);setQuery('');selectAcupoint(item.point.code);};
 ...
 const hideMuscle=(id:string)=>{if(!hideable(parts.get(id))||hiddenSet.has(id))return;setHiddenGroups(groups=>[...groups,[id]]);setState(s=>({...s,selected:[],isolate:false}));setChosen(null);setDetails(false);tutorial.showHintOnce('peel','한 층 벗기기는 선택한 구조만 숨깁니다. 되돌리기로 복원할 수 있어요.');};
 ...
 const openPanel=(next:'layers'|'search')=>{setDetails(false);if(next==='layers')closeNeedleEye();setPanel(p=>p===next?null:next);tutorial.openMiniTourOnce(next);};
 const prepareTutorial=()=>{if(insertionFrame.current!==null){cancelAnimationFrame(insertionFrame.current);insertionFrame.current=null;}reset();pinFocus(null);closeNeedleEye();setLocatorActive(false);setPanel(null);setAbout(false);setPanelPosition(null);setPanelCollapsed(false);setSheetDetent('peek');selectAcupoint('ST36',false);};
 const startTutorial=()=>{prepareTutorial();tutorial.start();};
 const replayTutorial=()=>{prepareTutorial();tutorial.replay();};
  const replayMiniTour=(id:MiniTourId)=>{setAbout(false);setDetails(false);if(id==='layers'){closeNeedleEye();setPanel('layers');}else if(id==='search')setPanel('search');else if(id==='locator'){if(!locatorActive)toggleLocator();}else{if(!needleEyeAvailable)selectAcupoint('GB34');setLocatorActive(false);setNeedleEyeOpen(true);}tutorial.showMiniTour(id);};
 const zoomScene=(factor:number)=>{setState(s=>({...s,zoomCommand:{factor,revision:++zoomRevision.current}}));if(factor<1)tutorial.orbit('zoom');};
 ...
 return <main className={`studio ${needleEyeOpen?'needle-eye-analysis':''} ${needleEyeWorkspace?'needle-eye-workspace':''} ${compact?'is-compact':''}`} style={{'--inset-top':`${viewInsets.top}px`,'--inset-right':`${viewInsets.right}px`,'--inset-bottom':`${viewInsets.bottom}px`,'--inset-left':`${viewInsets.left}px`} as React.CSSProperties}>
   {atlas&&<AnatomyScene atlas={atlas} state={{...state,needle,locatorGuide:locatorActive,inspectorOpen:details&&selectedParts.length>0,hiddenParts,viewInsets}} onSelect={choosePart} onPointSelect={code=>{if(code==='GB34'&&tutorial.step==='scroll'){tutorial.advance('scroll',compact?'finish':'landmark');selectAcupoint(code,'region');}else selectAcupoint(code,false);}} onUserCameraAction={kind=>tutorial.orbit(kind==='rotate'?'rotate':'zoom')} onProgress={n=>{setProgress(n);if(n===100)setError('');}} onError={setError} onNeedleReport={setNeedleReport} onAnnotationFrame={updateAnnotation} focus={focusChannel} onPathPartTap={focusPathPart}/>}
  <div className="vignette"/>
  <header className="identity"><div className="eyebrow"><span className="status-dot"/> FIRST-STUDY EDITION</div><h1>Acupoint Atlas<Badge variant="outline" className="edition">3D</Badge></h1><div className="identity-meta">{atlas?atlas.parts.length.toLocaleString():'—'} anatomical structures <span>·</span> 361 acupoints</div></header><section className="acupoint-rail" aria-label="Selected acupoint"><div className="eyebrow">SELECTED ACUPOINT · {meridian.label}</div><AcupointScrubber points={scrubPoints} activeCode={selectedGbPoint.code} onPointChange={code=>{if(code==='GB34'&&tutorial.step==='scroll')tutorial.advance('scroll',compact?'finish':'landmark');selectAcupoint(code);}} label={`${meridian.name} 경혈 탐색`} hotkeys/><div className="acupoint-rail-meta"><em>{selectedGbIndex+1} / {meridianPoints.length}</em></div><p className="acupoint-rail-hint">↑↓ Navigate · Esc Overview</p></section>
  {locatorActive
  ...
  <nav className="top-actions" aria-label="Explorer panels"><Button variant="ghost" className={panel==='search'?'active':''} onClick={()=>openPanel('search')} aria-label="Search anatomy" data-tip="검색|혈자리(코드·한글·한자)와 해부 구조 찾기 · 단축키 /"><Search size={18}/><span>Find a structure</span><kbd>/</kbd></Button><Button variant="ghost" className="icon-button" data-tutorial="help" aria-label="탐색 튜토리얼 다시 보기" data-tip="튜토리얼|사용법을 처음부터 다시 봅니다" onClick={replayTutorial}><HelpCircle size={18}/></Button><Button variant="ghost" className="icon-button" aria-label="About this atlas" data-tip="정보|출처와 학습 범위" onClick={()=>{setDetails(false);setPanel(null);setAbout(true);}}><Info size={18}/></Button></nav>
  ...
  <nav className="auxiliary-tools glass" aria-label="보조 해부 도구"><Button variant="ghost" className={locatorActive?'active':''} onClick={toggleLocator} aria-pressed={locatorActive} data-tutorial="locator" data-tip="취혈 가이드|기준이 되는 뼈와 표지점을 3D에 표시합니다"><Sparkles size={18}/><span>{locatorActive?'가이드 닫기':'취혈 가이드'}</span></Button><Button variant="ghost" onClick={()=>openPanel('layers')} aria-expanded={panel==='layers'} aria-controls="anatomy-layers" data-tip="해부 도구|계통별 레이어를 켜고 끄고, 모델을 분리합니다"><Layers3 size={19}/><span>해부 도구</span></Button><Button variant="ghost" onClick={()=>selected&&hideMuscle(selected.id)} disabled={progress<100||selectedParts.length!==1||!hideable(selected)||!state.visible.includes(selected!.system)||hiddenSet.has(selected?.id??'')} aria-label="선택한 구조 한 층 벗기기" data-tip="한 층 벗기기|선택한 구조 하나를 숨겨 그 아래를 봅니다" data-tip-off="먼저 3D에서 숨길 구조를 누르세요"><Layers3 size={18}/><span>한 층 벗기기</span></Button><Button variant="ghost" onClick={undoMuscleHide} disabled={!hiddenGroups.length} aria-label="마지막 숨김 되돌리기" data-tip="되돌리기|마지막으로 숨긴 구조를 다시 보이게 합니다" data-tip-off="아직 숨긴 구조가 없습니다"><Undo2 size={18}/><span>되돌리기</span></Button><FeedbackButton acupoint={selectedGbPoint.code} meridian={meridian.id} toolbar/><Button variant="ghost" aria-label="처음 상태로 초기화" data-tip="처음 상태로|시점과 레이어를 초기화합니다" onClick={reset}><RotateCcw size={18}/><span>초기화</span></Button></nav>
  ...
  {panel==='search'&&<section className="search-panel glass" aria-label="Find anatomy"><div className="panel-heading"><span>Find a structure</span><Button variant="ghost" className="icon-button" onClick={()=>setPanel(null)} aria-label="Close search"><X size={18}/></Button></div><Combobox<SearchItem> items={searchItems} autoHighlight value={null} onValueChange={value=>{if(value)chooseSearchItem(value);}} inputValue={query} onInputValueChange={setQuery} itemToStringLabel={item=>item.kind==="point"?`${item.point.code} ${item.point.korean}`:bilingualPartName(item.concept.name)} filter={null} open onOpenChange={open=>{if(!open)setPanel(null);}}><ComboboxInput autoFocus placeholder="GB34 · 양릉천 · 陽陵泉 · Heart…" aria-label="경혈(WHO 코드·한글명·한자명)과 해부 구조 검색" showTrigger={false}/><ComboboxContent className="anatomy-search-results"><ComboboxEmpty>No structures match your search.</ComboboxEmpty><ComboboxList>{(item:SearchItem)=>item.kind==="point"?<ComboboxItem key={item.id} value={item} className="search-result-point"><span className="search-result-name"><b>{item.point.code}</b> {item.point.korean} <small>{item.point.hanja}</small></span><span className="small-number">경혈</span></ComboboxItem>:<ComboboxItem key={item.id} value={item}><span className="search-result-name">{bilingualPartName(item.concept.name)}</span><span className="small-number">{item.concept.elements.length} {item.concept.elements.length===1?'piece':'pieces'}</span></ComboboxItem>}</ComboboxList></ComboboxContent></Combobox><p className="search-note">{query?'Showing up to 80 matches. Refine your search to find smaller structures.':'Start with a major organ, or search every named structure.'}</p></section>}
  <nav className="mobile-camera-controls glass" aria-label="모바일 카메라 조작">
   <Button variant="ghost" aria-label="몸 돌리기" data-tip="시점 전환|앞과 비스듬한 시점을 오갑니다" onClick={()=>{setState(s=>({...s,view:s.view==='front'?'three-quarter':'front',reset:s.reset+1,rotate:false}));tutorial.orbit('rotate');}}><RotateCw size={18}/></Button>
   <Button variant="ghost" aria-label="확대" data-tip="확대|화면 중심을 가까이" onClick={()=>zoomScene(.78)}><ZoomIn size={18}/></Button>
   <Button variant="ghost" aria-label="축소" data-tip="축소|멀리서 보기" onClick={()=>zoomScene(1.28)}><ZoomOut size={18}/></Button>
  </nav>
   ...
   <section ref={acupuncturePanel} className={`acupuncture-panel glass ${panelPosition&&!compact?'is-moved':''} ${panelCollapsed&&!compact?'is-collapsed':''} ${compact?`is-sheet sheet-${sheetDetent}`:''}`} style={compact?undefined:panelPosition??undefined} aria-label={`${meridian.name} 경혈`}>
    ...
    <MeridianRail tabs={availableMeridians.map(item=>({id:item.id,code:item.id,label:item.label,short:item.label.replace(/경$/,'')}))} activeId={meridian.id} disabled={!atlas} onCommit={id=>{const item=MERIDIANS[id as keyof typeof MERIDIANS];if(!item)return;if(id==='GB'&&tutorial.step==='meridian'){tutorial.advance('meridian','scroll');selectAcupoint('GB30',false);return;}selectAcupoint(item.first);}}/>
    ...
    <div ref={needlePanel} className={`needle-panel needle-${profile.region}`}>
      ...
      <div className="point-actions">
        ...
        <Button variant="ghost" data-tutorial="play" disabled={!needleReport?.available} onClick={animateInsertion}>자침 경로 재생</Button>
        <FeedbackButton acupoint={selectedGbPoint.code} meridian={meridian.id}/>
      </div>
      {profile.probeDepthMm<=0?<p className="strata-empty">이 혈은 자침 시뮬레이션을 제공하지 않습니다.</p>:needleReport&&needleReport.code===selectedGbPoint.code?<StrataColumn report={needleReport} ratio={needle.depthRatio} onRatio={ratio=>{updateNeedle(ratio);tutorial.advance('strata','needle-eye');}} refuseSignal={strataRefusal} focus={compassFocus}/>:<p className="strata-empty">경로를 계산하는 중입니다.</p>}
  ...
  <Sheet open={about} onOpenChange={setAbout}><SheetContent className="about-sheet glass"><div className="eyebrow">SOURCE & STUDY SCOPE</div><SheetTitle className="structure-title">해부·경혈 위치 학습용 모델</SheetTitle><SheetDescription>문헌 자침 범위를 단일 참조 모델에 기하학적으로 표시합니다.</SheetDescription><div className="about-copy"><p><strong>첫 학습본의 범위</strong><br/>이 화면은 성인 남성 참조 해부와 경혈 위치 학습용입니다. 진단, 시술 계획, 안전 심도 결정 또는 자가 자침 지침이 아닙니다.</p><p>{RELEASE_POINT_NOTE}</p><p>寸은 문헌 원문 그대로 보존하며 mm는 이 모델의 비례 환산값입니다. 자침 경로는 개인차·자세·호흡·질환을 반영하지 못하므로 실제 시술의 안전 판정으로 사용할 수 없습니다.</p><p>신경·혈관과 보완 메시도 경로 계산에 포함되지만, 웹용 메시 단순화와 정합 오차가 있어 실제 환자의 구조를 대신할 수 없습니다.</p><h3>상세설정</h3><button type="button" className="tutorial-replay-setting" onClick={replayTutorial}>탐색 튜토리얼 다시 보기</button>{(Object.keys(MINI_TOURS) as MiniTourId[]).map(id=><button key={id} type="button" className="tutorial-replay-setting" onClick={()=>replayMiniTour(id)}>{MINI_TOURS[id].name} 안내 다시 보기</button>)}<h3>색과 표시</h3><ul style={{margin:"8px 0",paddingLeft:18}}><li><b style={{color:"#1f8a7a"}}>청록</b> — 바늘이 지나는 층</li><li><b style={{color:"#d23c3c"}}>빨강</b> — 위험 구조(신경·혈관·장기 경계)</li><li><b style={{color:"#b8841f"}}>주황</b> — 취혈 기준 구조(표지점)</li><li>점선 — 모델에 없어 문헌으로 채운 층·구조</li></ul><h3>조작</h3><ul style={{margin:"8px 0",paddingLeft:18}}><li>회전은 드래그, 이동은 Shift 또는 오른쪽 드래그, 확대는 스크롤이나 두 손가락</li><li>혈 빠르게 넘기기: 혈 목록 위에서 스크롤, ↑↓ · PgUp/PgDn · Home/End</li><li>경락 바꾸기: 경락 레일을 끌거나 ←→</li><li>바늘 깊이: 층서 바를 끌거나 Needle’s Eye 위에서 스크롤</li><li>검색은 /, 닫기는 Esc</li><li>버튼 설명: 마우스를 올리거나 길게 누르기</li></ul><h3>권장 환경</h3><p>데스크톱, 노트북, 태블릿(iPad·Android)을 권장합니다. 휴대폰 화면은 아직 지원하지 않습니다.</p><h3>경혈 위치 기준</h3><p>혈 위치는 아래 순서로 판정했습니다. 앞의 자료가 우선하며, 자료끼리 충돌하면 그 내용을 해당 혈 설명에 표시했습니다.</p><ol style={{margin:"12px 0",paddingLeft:20}}><li>한국한의학연구원 한의약융합연구정보센터(KMCRIC) 경혈 위치 표준 문장</li><li>WHO Standard Acupuncture Point Locations in the Western Pacific Region (2008)</li><li>개발자의 실습 사진 대조 검수</li></ol><p>寸 표기는 원문을 그대로 두었고, mm는 이 모델 체형에 맞춘 비례 환산값입니다. 모든 임상 상황에 적용할 수 있는 모델이 아니며, 실제 임상에는 반드시 전문가의 판단이 필요합니다.</p><h3>Source</h3><p>BodyParts3D(© The Database Center for Life Science, CC BY-SA 2.1 JP)와 Z-Anatomy(CC BY-SA 4.0)를 바탕으로 학습용으로 다시 구성했습니다. 원본 메시는 단순화와 정합을 거쳤고, 일부는 보완했습니다.</p><a href="https://dbarchive.biosciencedbc.jp/en/bodyparts3d/lic.html" target="_blank" rel="noreferrer">Dataset license <ArrowUpRight size={14}/></a><a href="https://dbarchive.biosciencedbc.jp/en/bodyparts3d/download.html" target="_blank" rel="noreferrer">Original geometry & metadata <ArrowUpRight size={14}/></a><a href="https://academic.oup.com/nar/article/37/suppl_1/D782/1000752" target="_blank" rel="noreferrer">Read the source publication <ArrowUpRight size={14}/></a><h3>만든 사람</h3><p><strong>J.H. Song</strong> · WKU College of Korean Medicine<br/>개인 학습 프로젝트이며, 소속 학교의 공식 자료나 감수 자료가 아닙니다.<br/>감수: 없음<br/>문의: 화면의 피드백 버튼</p><a href="https://github.com/JIHO-0629" target="_blank" rel="noreferrer">GitHub · JIHO-0629 <ArrowUpRight size={14}/></a><h3>오류를 발견하면</h3><p>혈 위치나 구조 이름이 이상하면 해당 혈을 선택한 상태에서 피드백 버튼을 눌러 주세요. 선택한 혈과 화면 정보가 함께 전달됩니다. 개인 프로젝트라 학술적·기능적 오류가 있을 수 있으며, 계속 업데이트하고 있습니다. 언제든 의견 주시면 감사하겠습니다.</p><p style={{fontSize:"var(--text-caption)",lineHeight:1.6,color:"#87929d"}}>버전 0.1.0 · 최종 검수 2026-09-26<br/>인용: Song, J.H. 《Acupoint Atlas: 해부·경혈 위치 학습용 3D 모델》 (v0.1.0). https://human-atlas-seven.vercel.app, 접근일 기재.</p></div></SheetContent></Sheet>
  {tutorial.step&&<Tutorial step={tutorial.step} compact={compact} onStart={startTutorial} onSkip={()=>tutorial.close('skipped')} onDone={()=>tutorial.close('complete')}/>}
  {tutorial.hint&&<FeatureHint text={tutorial.hint} onClose={tutorial.clearHint}/>}
  {tutorial.miniTour&&!tutorial.step&&<MiniTour id={tutorial.miniTour.id} index={tutorial.miniTour.index} onNext={tutorial.nextMiniCard} onClose={tutorial.closeMiniTour}/>}
 </main>;
}
